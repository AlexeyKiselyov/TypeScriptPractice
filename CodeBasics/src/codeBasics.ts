...
// ---------> 51 tasks ---> Index Signature (динамические ключи)
...
interface EmployeeSalary2 {
  [key: string]: number;
}

function buildSalaryStatistics2(employees: EmployeeSalary2): SalaryStatistics {
  const result: SalaryStatistics = {
    min: 0,
    max: 0,
    avg: 0,
  };
  ...
  let sumOfAllSalary = 0;

  employeesKeys.forEach(key => {
    sumOfAllSalary += employees[key];

    if (result.min === 0) {
      result.min = employees[key];
      result.max = employees[key];
      return;
    }

    if (result.min > employees[key]) {
      result.min = employees[key];
      return;
    }

    if (result.max < employees[key]) {
      result.max = employees[key];
      return;
    }
  });

  result.avg = sumOfAllSalary / employeesKeys.length;

  return result;
}

// ---------> 52 tasks ---> Сопоставление типов (Mapped Types)
